...
func (db *Db) SaveBridgeTransaction(tx *types.BridgeTransaction) (int64, error) {
	stmt := `
	INSERT INTO bridge_transaction (operation_unique_id, unique_key, height, user_initiated_hash, msg_index, source_chain, destination_chain, sender, recipient, denom, amount)
...
func (db *Db) SaveBridgeEvidence(evidence *types.BridgeEvidence) (int64, error) {
	stmt := `
	INSERT INTO bridge_evidence (tx_unique_key, height, hash, msg_index, relayer_address, threshold_reached, settlement_hash, result)
	VALUES ($1, $2, $3, $4, $5, $6, $7::TEXT, $8)
	ON CONFLICT (hash, msg_index) DO UPDATE SET
		tx_unique_key = EXCLUDED.tx_unique_key,
		height = EXCLUDED.height,
