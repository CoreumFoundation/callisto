...
func NewSource(source *local.Source, queryClient wasmtypes.QueryClient) *Source {
	return &Source{
		Source:      source,
...
func (s *Source) GetSendToXRPLOperationIDs(
	...
	beforeCtx, err := s.LoadHeight(int64(height - 1))
	if err != nil {
	...
	afterCtx, err := s.LoadHeight(int64(height))
	if err != nil {
		return nil, fmt.Errorf("error while loading height: %s", err)
	}
